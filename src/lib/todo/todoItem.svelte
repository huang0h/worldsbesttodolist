<script lang="ts">
	...
	function removeSubtask(item: Task) {
		return (taskToRemove: string) =>
			(item.subtasks = item.subtasks.filter((task) => task.action !== taskToRemove));
	}
</script>
...
<ul>
	<li>
		<p>
			<button class="complete-button" onclick={() => parentRemoveSubtask(itemState.action)}
				>✅</button
			>
			{itemState.action}
